...
function flagImage(imagePath){
	...
	if(lable != undefined && lable != null){
			lable.innerHTML = "<img src=\""+imagePath+"/"+ direction +".gif\" border=\"0\">";
	}
...
function bindTitle(fieldArray,bodyName){
	var pField;
	var fieldName = "";	
	if (document.getElementById("theObjTable").getElementsByTagName("td")[0] == undefined){			
		...
		strTag = "<table width='100%' border='1' cellpadding='0' cellspacing='0' id='theObjTable' style='table-layout:fixed'><tbody id='"+bodyName+"'><tr bgcolor='#CAD9EC' height='22'>"+strTag+"</tr></tbody></table>";
	...
	for(h=0;h<test.getElementsByTagName("td").length;h++){
			...
			test.getElementsByTagName("td")[h].style.borderBottom = "1px solid #86a1d1";
			test.getElementsByTagName("td")[h].style.borderRight = "1px solid #2A529C";
			...
			test.getElementsByTagName("td")[h].innerHTML="";
...
function initGrid(fieldArray,cssFile,imagePath,isCursor,tBodyName){
...
setCssFile(cssFile);
if (bindTitle(fieldArray,bodyName) == -1){return; }
flagImage(imagePath);
