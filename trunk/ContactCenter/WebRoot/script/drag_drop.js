...
a.a.offsetHeight-a.e.offsetHeight+"px";b.style.top=a.e.offsetHeight+"px";a.a.appendChild(b)}};ig_.$=function(){var a=[ig_.ha,ig_.ja];for(var b=0;b<a.length;b++){var c=_gel(a[b]);if(c){c.parentNode.removeChild(c);c=null}}};ig_.db=function(){var a="";for(var b=0;b<ig_.q.length;b++){var c=ig_.q[b];for(var d=0;d<c.childNodes.length-1;d++){var e=c.childNodes[d];if(e.tagName=="DIV"){a+=a!=""?":":"";a+=e.id.substring(2)+"_"+c.id.substring(2)}}}_xsetp("mp="+_esc(a))};function ig_Y(a){this.x=ig_Z;this.a=a;
...
function ig_S(){ig_.$();if(this.O()){ig_.db()}if(this.m){var a=this.m.id.match(/tab(\d+)_/)[1],b=this.a.id.match(/m_(\d+)/)[1];_xsetp("mt="+b+":"+a);this.a.style.display="none";this.m.className=this.m.className.replace(/ tab_hover/g,"")}if(!_uli){var c=_gel("new_user_tip");if(c){c.style.display="none"}var d=_gel("new_user_save");if(d){d.style.display="block"}}if(this.i){this.E("dragend")}return true}
...
function ig_5a(a,b){return function(){var c=ig_cb(),d=ig_G(),e=d[1],f=0,g=0.05*c,h=e,i=a.offsetTop;if(a.lastMouseY<=g){i=a.offsetTop-f;h=e-f}else if(a.lastMouseY>=c-g){i=Math.min(b-a.offsetHeight,a.offsetTop+f);h=Math.min(b-c,e+f)}var j=h-e;if(j!=0){window.scrollBy(0,j);a.style.top=i+"px"}}}
