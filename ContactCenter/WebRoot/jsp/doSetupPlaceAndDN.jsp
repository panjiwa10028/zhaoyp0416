...
.nColor{
	...
	<!--
		...
		function refresh()
		{
			...
			{
				location.href = "<%=strContextPath%>/setupPlaceAndDn.do?method=listPlace&strTenantId=" + strTenantId.value + "&lPlaceCTIId=" + lPlaceCTIIdTemp + "&strPlaceName=" + strPlaceNameTemp + "&nCTIUseState=" + nCTIUseStateTemp + "&lVoiceDNCTIId=" + lVoiceDNCTIId.value + "&lEmailDNCTIId=" + lEmailDNCTIId.value + "&lChatDNCTIId=" + lChatDNCTIId.value + "&lLoginTemplateId=" + lLoginTemplateIdTemp;
		...
		//按照选择的PlaceId显示DN
		function changePlaceId(id,name,state)
		{
...
<body style="overflow:auto" onLoad="loadInfo();aa()">
...
<div id="contentArea" style="height:; overflow:auto; margin-bottom:8px; border:1px solid #cccccc; background-color:#FFFFFF;">
    ...
    <tr>
      <td><table border="0" align="center">
          <tr>
            ...
            <td width="24">&nbsp;</td>
            <td class="nobreak"><%=Function.getMultiLangItemContent("setupPlaceAndDN.loginTemplate", request)%>
              <select name="lLoginTemplateId1" class="inputBox" style="width:100px">
                <c:forEach var="template" items="${LOGINTEMPLATELIST}">
            ...
            <td class="nobreak"><%=Function.getMultiLangItemContent("setupPlaceAndDN.place", request)%>
              <input id="lPlaceCTIId1" name="textfield" type="text" class="inputBox" size="16" onClick="shDiv('selTel');event.cancelBubble = true;" onChange="refresh()" <c:if test="${isExist=='true'}">value="${param.strPlaceName}"</c:if> />
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="0"
						...
						<tbody id="dataArea">
							<c:forEach var="place" items="${PLACELIST}" varStatus="status">
								<tr>	
								<c:set var="strNameTemp" value="${place.STRNAME}" />
								<c:set var="strNameTemp1" value="${place.STRDESCRIPTION}" />							
									<td <c:if test="${place.LAGENTID != 0}">bgcolor="#dbdde0"</c:if><c:if test="${place.STRNAME == param.strPlaceName}">bgcolor="#6296d7"</c:if> onclick="changePlaceId('<c:out value="${place.LDBID}" />','<%=Function.formatToScript((String)pageContext.getAttribute("strNameTemp"))%>','<c:out value="${place.NPLACESTATE}" />','<%=Function.formatToScript((String)pageContext.getAttribute("strNameTemp1"))%>')">
  ...
  <div id="dnTable1" style="display:none;"> 
  ...
  <table id="dnTable" width="95%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="8fa9d5">
    ...
    <c:forEach var="dn" items="${list}" varStatus="status">
    	<c:if test="${dn.NCHANNELID==1 || (dn.NCHANNELID!=1 && nAILVersion==65 && strAILAvailable=='1')}"> 
    <tr bgcolor="eef5fe">
      <td width="25" align="center"><input type="checkbox" id="lCTIDNId<c:out value="${dn.LDBID}" />" name="lDNId" value="<c:out value="${dn.LDBID}" />" nChannelId="<c:out value="${dn.NCHANNELID}" />"/></td>
      <input type="hidden" id="strDNId" name="strDNId" value="<c:out value="${dn.STRDNID}" />">
      <td>
		<c:if test="${dn.NCHANNELID==1}"><%=Function.getMultiLangItemContent("common.channel.telephone", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==2}"><%=Function.getMultiLangItemContent("common.channel.email", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==3}"><%=Function.getMultiLangItemContent("common.channel.fax", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==4}"><%=Function.getMultiLangItemContent("common.channel.sms", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==5}"><%=Function.getMultiLangItemContent("common.channel.mail", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==6}"><%=Function.getMultiLangItemContent("common.channel.chat", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==7}"><%=Function.getMultiLangItemContent("common.channel.cobrowser", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
		<c:if test="${dn.NCHANNELID==8}"><%=Function.getMultiLangItemContent("common.channel.voip", request)%>:<c:out value="${dn.STRDNID}" /></c:if>
      </td>
      <td width="80" align="center">
      	<c:if test="${dn.NDNSTATE==0}"><%=Function.getMultiLangItemContent("common.DN.state.logout", request)%></c:if>
		<c:if test="${dn.NDNSTATE==1}"><%=Function.getMultiLangItemContent("common.DN.state.login", request)%></c:if>
		<c:if test="${dn.NDNSTATE==2}"><%=Function.getMultiLangItemContent("common.DN.state.notready", request)%></c:if>
		<c:if test="${dn.NDNSTATE==3}"><%=Function.getMultiLangItemContent("common.DN.state.ready", request)%></c:if>
      </td>
    </tr>
    </c:if>
    </c:forEach>
    <c:if test="${nAILVersion>=72 && strAILAvailable=='1'}">
    <tr bgcolor="#FFFFFF">
      <td width="25" align="center"><input type="checkbox" id="lCTIDNIdEmail" name="lDNId" value="0" nChannelId="2"></td>
      <td><%=Function.getMultiLangItemContent("setupPlaceAndDN.Email", request)%></td>
      <td width="80" align="center"><%=Function.getMultiLangItemContent("common.DN.state.logout", request)%></td>
    </tr>
    <tr bgcolor="eef5fe">
      <td width="25" align="center"><input type="checkbox" id="lCTIDNIdChat" name="lDNId" value="0" nChannelId="6"></td>
      <td><%=Function.getMultiLangItemContent("common.channel.chat", request)%></td>
      <td width="80" align="center"><%=Function.getMultiLangItemContent("common.DN.state.logout", request)%></td>
    </tr>
    </c:if>
  </table>
