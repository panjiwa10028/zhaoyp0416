...
<%
	String strProductName = Function.getMultiLangItemContent((String)request.getAttribute("strProductName"),request);
...
<script>
	moveTo(0,0);//移动窗体
	resizeTo(screen.availWidth,screen.availHeight);//根据分辨率改变窗体大小

var lPresetOutboundBusinessId="";//预设外呼业务Id	
var d1= new Date();
var strProductId='<c:out value="${strProductId}"/>';//产品ID
...
var strManDNId='<c:out value="${STROFFICEPHONEEXT}"/>';//Add by Alex
var strSkillGroupId='<c:out value="${STRGROUPHEADERID}"/>';//Add by Alex
var strDepartmentHeaderId='<c:out value="${STRDEPARTMENTHEADERID}"/>';//Add by Alex
var strExt = "${STREXT}";
//国际化字符串
//info add by alex begin
...
var localeManagerForceInterveneFaile = '<%=Function.getMultiLangItemContent("transactInfo", request)%><%=Function.getMultiLangItemContent("LocaleManager.AgentSee.New.forceIntervene.faile", request)%>';
...
//info add by alex end

var tempWinObj;
var CampaignExecutionManagerName = "<%=Function.getMultiLangItemContent("Campaign.CampaignExecutionManager.Name",request)%>";
var CampaignExecutionAgentGroup = "<%=Function.getMultiLangItemContent("Campaign.CampaignExecutionManager.UI.AgentTaskFrom.AgentGroup",request)%>";
var CampaignExecutionAgent = "<%=Function.getMultiLangItemContent("Campaign.CampaignExecutionManager.UI.AgentTaskFrom.Agent",request)%>";
var tempDir="";
...
//0=没有pin float,1=pin float
var isPinFloat = false;
//记住弹出窗是否浮动，0为嵌入，1为浮动，2为关闭
var isMesFloat=1;
//记住Faq弹出窗是否浮动，0为嵌入，1为浮动，2为关闭
var isFaqFloat=2;
...
function getLeftTree()
{
	...
	<c:forEach var="moduleBean" items="${moduleList}"  varStatus="status">
		...
		contentAreaURL = '<c:out value="${moduleBean.contentAreaURL}"/>';
		...
		<c:if test="${moduleBean.moduleLevel==0}">
			...
			<c:if test='${strProductId=="AdminCenter"}'>
				...
				sHtml.append('<%=Function.getMultiLangItemContent((String)pageContext.getAttribute("strName_1"),request)%>');
				sHtml.append("</div>");
			</c:if>
		...
		<c:if test="${moduleBean.moduleLevel ==1}">
			...
			if(subfirst==1)
			{
				addTab('ipcc','<%=Function.getMultiLangItemContent((String)pageContext.getAttribute("strNameTemp"),request)%>','<%=strContextPath%>/jsp/main/System.jsp',false,true,false,treeBarURL,contentAreaURL);
			...
			<c:if test="${moduleBean.reserveFlag!=-1}">
		     ...
		     sHtml.append("onClick=\"top.addTab('"+ModuleName+"',");
				...
				sHtml.append("'<%=Function.getMultiLangItemContent((String)pageContext.getAttribute("strNameTemp"),request)%>',");
			...
			//alert(sHtml);
				...
				<c:if test="${moduleBean.imageFile!=''}">
					...
					sHtml.append(" width=\"16\" height=\"18\" border=\"0\" align=\"absmiddle\"><%=Function.getMultiLangItemContent((String)pageContext.getAttribute("strNameTemp"),request)%></span></td></tr>");
				</c:if>
				...
				sHtml.append("<%=Function.formatShortToHTML(Function.getMultiLangItemContent((String)pageContext.getAttribute("strNameTemp"),request),30)%></span></td></tr>");
...
function loadInfo()
{
	...
	viewTime();
	//取得queue的值
	strStatServerAvailableThisPage=0;
...
<body scroll="no" border="no" onbeforeunload="confirmLogoutProduct();" onunload="logoutProduct('sysclose');">
<!-- 隐藏变量 -->
		...
		<tr>
			<td height="30">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" height="30">
					<tr>
						<td width="13"><img src="<%=strContextPath%>/images/leftTree_top1.gif" width="13" height="30"></td>
						<td background="<%=strContextPath%>/images/leftTree_top2.gif"><div id="menuWord" style="font-size:14px; font-weight:bold; padding-top:5px;cursor:default"  class="nobreak"></div></td>
						<td width="7"><img src="<%=strContextPath%>/images/leftTree_top3.gif" width="7" height="30" border="0"></td>
					</tr>
				</table>
			</td>
		</tr>
		<tr>
			<td valign="top" style="background-image:url(<%=strContextPath%>/images/leftTree_1.gif);background-repeat:repeat-y;background-position:left; padding-left:6px;">
				<!-- 导航栏 -->
						...
						<c:if test="${moduleBean.moduleLevel==0}">	
							<c:if test="${moduleBean.personalFlag==0}">	
								<div name="menu_${moduleBean.moduleId}" id="mt<%=i%>" <c:choose><c:when test="${moduleBean.isFirstFlag==1}">class="menuTitleDown"</c:when><c:otherwise>class="menuTitle"</c:otherwise></c:choose> onClick="clickMenu('mc<%=i%>')" onMouseOver="overMenu(this)" onMouseOut="outMenu(this)">
									<img src="<%=strContextPath%>/<c:out value="${moduleBean.imageFile}"/>" width="16" height="16" border="0" align="absmiddle">  <span><c:set var="strName_1" value="${moduleBean.multiLangItemId}" /><%=Function.getMultiLangItemContent((String)pageContext.getAttribute("strName_1"),request)%></span>
								</div>
							</c:if>
...
<c:choose>
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="0"id="miniTopDiv" style="display:none; height:35px; background-image:url(<%=strContextPath%>/images/logo35_bg.gif); background-position:bottom">
  	<tr>
		...
		<div id="topDiv" class="ContactTopDiv">
			<table width="100%" border=0 cellPadding=0 cellSpacing=0>
				<tr>
					<td width="205" rowspan="2" valign="bottom" background="<%=strContextPath%>/images/Top_logobg.gif">
						<table width="205" border="0" cellpadding="0" cellspacing="0">
							<jsp:include page="/softphone/stateIcon.jsp" flush="true"/>
						</table>
					</td>
				  	<td width="99%" height="22" valign="top" background="<%=strContextPath%>/images/Top_lab1bg.gif">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td style="background-image:url(<%=strContextPath%>/images/Top_lab1_left.gif); background-repeat:no-repeat; background-position:left" height="22" align="right" class="nobreak">
									<!--<span><c:out value="${STRUSERNAME}"/>,<c:out value="${nowTime}"/>&nbsp; 
										<c:if test="${!empty MEDALNAME}">
											<img src="<%=contextPath%>/FileManage/downloadFileServlet?strServiceName=MedalManager&strTransName=GetMedalAttachedFile&strFieldName=binImage&lId=<c:out value="${MEDALID}"/>&lUserId=<c:out value="${LUSERID}"/>&lTenantId=<c:out value="${LTENANTID}"/>&strFileName=<c:out value="${STRFILENAME}"/>" width="16" height="16">
										</c:if>
									&nbsp;&nbsp;
									</span>-->
								</td>
				...
				<tr>
					<td height="73" valign="top" background="<%=strContextPath%>/images/Top_lab2bg1.gif">
						<table width="100%"  border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><img src="<%=strContextPath%>/images/Top_lab1_corner.gif" width="45" height="7" border="0" align="absmiddle"></td>
							</tr>
						</table>
						<table width="100%" border="0" cellpadding="0" cellspacing="0">      
							<tr>    
								...
								<td>
									<table border="0" cellpadding="0" cellspacing="0"> 
										<tr>
											<td><jsp:include page="/softphone/mediaIcon.jsp" flush="true"/></td>
						...
						<table width="100%"  border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><img src="<%=strContextPath%>/images/Top_lab1_corner2.gif" width="24" height="9" border="0" align="absmiddle"></td>
							</tr>
						</table>
					</td>
				</tr>
...
<!-- 下面是popTable1 >
<table border="0" cellpadding='0' cellspacing='0' onmousedown="Focus(this)" id="popTable1" name="popTable1" style="position:absolute; display:none1; top:225px; left:345px; width:620px; height:140px; z-index:1010; table-layout:fixed; border:1px solid #7191c1; background-color:#ffffff;">
	<tbody id="popTableTitle" onmousedown="setPhoneNumberFocus();">
		<tr style="cursor:default;" onmousedown="Down(this)" onmousemove="Remove(this)" onmouseup="Up(this)">
			<td height="20" background="<%=strContextPath%>/images/popTitleBg.gif" style="height:20px;">
				<table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td style="padding-left:12px; white-space:nowrap; overflow:hidden"><%=Function.getMultiLangItemContent("BusinessLayout.LinkMessage", request)%></td>
						<td align="right" style="white-space:nowrap; overflow:hidden"><img src="<%=strContextPath%>/images/st_gotop.gif"border="0" align="absmiddle" style="cursor:pointer;" onClick="goTop()" alt="<%=Function.getMultiLangItemContent("BusinessLayout.pop.dock", request)%>"> <img src="<%=strContextPath%>/images/st_close17.gif" border="0" align="absmiddle" style="cursor:pointer; margin-left:3px;"  onClick="closeFloat()" alt="<%=Function.getMultiLangItemContent("BusinessLayout.pop.hide", request)%>"> </td>
					</tr>
				</table>
			</td>
			<td width="16" height="20" align='right'  background="<%=strContextPath%>/images/popTitleBg.gif"></td>
		</tr>
	</tbody>
		<tr>
			<td colspan="2"  style="overflow:hidden;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" id="popFrame" style="table-layout:fixed;">
					<tr onDblClick="dbClickBar()">
						<td background="<%=strContextPath%>/images/pop_tab_bg.gif">
							<table height="24" border="0" cellpadding="0" cellspacing="0" id="mesIconTable">
								<tr><td width="12" id="nn"></td></tr>
							</table>
						</td>
						<td width="45" align="right" background="<%=strContextPath%>/images/pop_tab_bg.gif">
							<span style="display:none; padding-right:5px; text-align:right" id="onTop"><img src="<%=strContextPath%>/images/st_pop.gif" border="0" align="absmiddle"  style="cursor:pointer"  onClick="goFloat()" alt="<%=Function.getMultiLangItemContent("BusinessLayout.pop.undock", request)%>">  <img src="<%=strContextPath%>/images/st_close17.gif" border="0" align="absmiddle" style="cursor:pointer; margin-left:3px;"  onClick="closeFloat()" alt="<%=Function.getMultiLangItemContent("BusinessLayout.pop.hide", request)%>"></span>
						</td>
					</tr>
				</table>
				</td>
		...
		<tr>
			<td colspan="2" valign="top" height="100%" style="padding-left:1px;">
				<table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0" id="popIframeTable">
					<tr><td></td></tr></table></td>
		</tr>
	<tbody id="inHandle" style="display:none;">
		<tr>
			<td colspan="2" style="cursor: s-resize; height:4px; background-image:url(<%=strContextPath%>/images/dragGrid1.gif)" onmousedown="Down(this)" onmouseup="Up(this)" onmousemove="ResizeH(this)"><img src="<%=strContextPath%>/images/dragGrid_handle.gif" style="position:absolute; margin-top:-2px; width:100%; height:5px"></td>
		</tr>
	</tbody> 
	<tbody id="floatHandle" style="display:">
		<tr>
			<td  style="cursor:n-resize; height:0px;" onmousedown="Down(this)" onmouseup="Up(this)" onmousemove="ResizeVertical(this)"><img style="position:absolute; margin-top:-5px;" src="<%=strContextPath%>/images/null.gif" width="100%" height="10" border="0" align="absmiddle"></td>
			<td style="cursor:se-resize; height:0px;" onmousedown="Down(this)" onmouseup="Up(this)" onmousemove="Resize(this)"><img style="position:absolute; margin-top:-5px;" src="<%=strContextPath%>/images/null.gif" width="18" height="16" border="0" align="absmiddle"></td>
		</tr>
	</tbody> 
</table>
< popTable1结束 -->
...
<!-- popTable2开始 >
<table border="0" cellpadding='0' cellspacing='0' onmousedown="Focus(this)" id="popTable2"  name="popTable2" style="position:absolute; display:none1; top:243px; left:189px; width:700px; height:320px; z-index:1010; table-layout:fixed; border:1px solid #7191c1; background-color:#ffffff;">
	<tr style="cursor:default;" onmousedown="Down(this)" onmousemove="Remove(this)" onmouseup="Up(this)">
		<td height="20" background="<%=strContextPath%>/images/floatRightDivTitleBg.gif">
			<table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td style="padding-left:12px; white-space:nowrap; overflow:hidden">FAQ</td>
					<td align="right" style="white-space:nowrap; overflow:hidden"><a href="javascript:void(0)" onClick="goBottom('common/faq.htm')">goBottom</a> <a href="javascript:void(0)" onClick="hideDiv('popTable2');isFaqFloat=2">Close</a> </td>
				</tr>
			</table>
		</td>
		<td width="14" height="20" align='right'  background="<%=strContextPath%>/images/floatRightDivTitleBg.gif"></td>
	</tr>
	<tr>
		<td colspan="2" valign='top' style="height:100%;"><iframe id="popFaqFrame" src="about:blank" allowtransparency="yes" frameborder="0" height="100%" width="100%" hspace="0" longdesc="0" marginheight="0" marginwidth="0" scrolling="auto" vspace="0"></iframe> 
		</td>
	</tr>
	<tr>
		<td></td>
		<td width="16" valign="bottom" align="right" style="cursor:se-resize; height:16px; width:16px;" onmousedown="Down(this)" onmouseup="Up(this)" onmousemove="Resize(this)"><img src="<%=strContextPath%>/images/resizeHandle.gif" width="14" height="14" border="0" align="absmiddle"></td>
	</tr>
</table>
<popTable2结束 -->
...
<!-- 以下部分由stella添加 -->
<c:if test='${strProductId==""}'>  
						<td width="100" height="36" style="padding-top:6px">
							<div id="strQueue" style="display:none">
								<table id ="t_queue" border="0" cellpadding='0' cellspacing='0' onmousedown="Focus(this,'queue')" id="popTable" name="popTable" style="position:absolute;width:90px; height:24px; z-index:1010; table-layout:fixed; border:1px solid #ffffff; background-color:#000000;filter:alpha(opacity=80);">
									<tr>
										<td onmousedown="Down(this,'queue')" onmousemove="Remove(this,'queue')" onmouseup="Up(this,'queue')" style="color:#00CC00; font-size:12px; padding-left:4px; cursor:move; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;	-o-text-overflow:ellipsis;">
										<!-- marquee direction="up" id="message" scrolldelay="150" onMouseOver="this.stop()" onMouseOut="this.start()" style="width:100%;height:23px" scrolldelay="1000" scrollamount="1"></marquee--><span id="message"></span></td>
...
<!-- stella添加部分结束 -->
			...
			<td>
					...
					<div id="processMessage" style="display:none; position:absolute;border:1px solid #00aeda;background-color:#c5f1ff;padding:5px;width:expression(document.body.clientWidth-240);margin-top:5px;margin-left:-3px;filter: alpha(opacity=80);">
						<table width="100%"  border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td id="statusBarInfo">&nbsp;</td>
								<td width="15" align="right">
									<img src="<%=strContextPath%>/images/close1.gif" width="13" height="13" style="cursor:pointer;" title="Close" onClick="hideDiv('processMessage')">
								</td>
							</tr>
						</table>
					</div>
