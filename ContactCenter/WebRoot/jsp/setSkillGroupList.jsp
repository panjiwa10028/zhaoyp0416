...
function submitForm() {				
			...
			function loadInfo() {
				...
				var cookieId = '${param.strSkillGroupCookieId}';
  ...
  <tr>
    <td style="padding-left:20px"><strong><%=Function.getMultiLangItemContent("CCM.Login.Page.SetSkillGroup",request)%></strong><span id="testX"><span></td>
  </tr>
</table>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tbody id="dataArea">
					<tr>
						...
						<td>
							<%=Function.getMultiLangItemContent("CCM.Login.Page.SoftPhoneIsNotLogin",
									request)%>
						</td>											
					</tr>
					<c:forEach var="element" items="${GROUPRECORDSET}"  varStatus="status">
					<tr>
						<td align="center">
								<input class="checkbox" type="radio" id="${element.LID}" name="lId"
											value="<c:out value='${element.LID}'/>"											
								<c:if test="${STRSKILLGROUPCOOKIEID == element.LID }">checked</c:if>/>
						</td>
						<td align="center">
							<c:out value='${status.index + 2}' />
						</td>
						<td>
							<c:out value="${element.STRNAME}"/>
						</td>											
					</tr>
					</c:forEach>
...
["",25,,,],
["",20,,,],
["<%=Function.getMultiLangItemContent("CCM.Login.Page.Name", request)%>",,"",""]
];
