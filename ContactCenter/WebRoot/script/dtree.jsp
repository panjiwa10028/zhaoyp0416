...
function text2Html_View(strText)
{
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useControl==true)
	{
		...
		{// 不为根节点或设置了根节点使用控件生成控件
			// 使用控件，生成html代码
			str += '<input id="ctrl' + this.obj + nodeId + '" style="width:15px; height:15px" name="';
...
function showModalDialogTree(tree, inputCtrlId)
{
	...
	window.showModalDialog("<%=request.getContextPath()%>/jsp/commonTreeIframe.jsp", array, 'dialogWidth=300px;dialogHeight=520px;status:no;resizable:no;help:no;scroll:no');
}
...
// get lId(its parentId = 0) from the tree for a specific node
dTree.prototype.getBranchRootId = function(nId, isType) {
	for(var i = 0; i < this.aNodes.length; i++)
	{
		if(this.aNodes[i].value==nId && this.aNodes[i].isType==isType)
		{
	...
	if(i < this.aNodes.length)
	{	
		return this.getParentRootId(this.aNodes[i]._ai, true);
	}
};

dTree.prototype.getParentRootId = function(nId, bFirst) {
	...
	if(cn._p._ai == 0)
	{
		...
		return this.getParentRootId(cn._p._ai, true);	
};
